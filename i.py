...
class Packet:
    ...
    def get_Header(self):
        hex_string = str(self.header.hex())
...
def writeFilePNG(filePath, hexTS):
    pngSignature = bytes.fromhex("89 50 4E 47 0D 0A 1A 0A")
    IHDR = bytes.fromhex(
        "00 00 00 0D 49 48 44 52 00 00 00 01 00 00 00 01 08 06 00 00 00 1F 15 C4 89"
    )
    IDAT = bytes.fromhex(f"00 00 FF A5 49 44 41 54 {hexTS} ce b3 26 6a")
    IEND = bytes.fromhex("00 00 00 00 49 45 4e 44 ae 42 60 82")
    pngFake = pngSignature + IHDR + IDAT + IEND
    with open(filePath, "wb") as f:
        f.write(pngFake)


# readFilePNG("assets\demo.png")
# readFileTS("assets\segment_1.ts")
def getHexFromTS(filePath):
    with open(filePath, "rb") as f:
        s = f.read()
        return s.hex()


def getHexTSFromPNG(filePath):
    with open(filePath, "rb") as f:
        # ignore 16 bytes
        f.read(32)
        data = f.read()
...
def writeFileTS(filePath, hexTS):
    tsFake = bytes.fromhex(hexTS)
    with open(filePath, "wb") as f:
        f.write(tsFake)
